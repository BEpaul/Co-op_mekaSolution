...
def getContent(driver):
    ...
    # comments
    try:
        comments1 = soup.select('EtaWk.C4VMK')[0].text
        comments2 = soup.select('TCSYW.EizgU')[7].findAll('span').text
        
        comments_data.append(comments1)
        comments_data.append(comments2)
        # access content by its html tag and store them as text string
    except:
        comments1 = None
        comments2 = None
    ...
    post_data = [like, content, date, place, instagram_tags, num]
...
follower_output.to_csv('./instagram_crawling_follower3_data.csv')
# save the output data as csv file (can be accessed using excel or notepad)

data_output = pd.DataFrame(crawling_data, columns = ['like', 'content', 'date', 'place', 'tags', 'num'])
# to handle large amount of data, use Dataframe supported by pandas
data_output.to_csv('./instagram_crawling_post_data3.csv',encoding='utf-8')
# save the output data as csv file (can be accessed using excel or notepad)